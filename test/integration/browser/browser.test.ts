...
describe('Browser tests', () => {
    ...
    test('Raster tiles finish fading after zooming with no elapsed time before the first draw', {timeout: 20000}, async () => {
        const pixels = await page.evaluate(async () => {
            async function tileImage(color: string): Promise<ArrayBuffer> {
            ...
            const parentImage = await tileImage('#ff0000');
            const childImage = await tileImage('#0000ff');
            maplibregl.addProtocol('fade-test', async ({url}) => ({
                data: (url.startsWith('fade-test://0/') ? parentImage : childImage).slice(0)
            }));
            map.remove();
            map = new maplibregl.Map({
                ...
                style: {
                    version: 8,
                    sources: {raster: {type: 'raster', tiles: ['fade-test://{z}/{x}/{y}'], tileSize: 512, maxzoom: 1}},
                    layers: [{id: 'raster', type: 'raster', source: 'raster', paint: {'raster-fade-duration': 300}}]
                }
            });
            await map.once('idle');
            const beforeZoom = readPixel();
            const fadeStart = maplibregl.now() + 1000;

            try {
                maplibregl.setNow(fadeStart);
                map.setZoom(1);
                await new Promise<void>(resolve => {
                    function onRender() {
                        if (!map.isSourceLoaded('raster')) return;
                        map.off('render', onRender);
                        resolve();
                    }
                    map.on('render', onRender);
                });
                const firstFrame = readPixel();
                maplibregl.setNow(fadeStart + 400);
                const nextFrame = map.once('render');
                map.triggerRepaint();
                await nextFrame;
                const afterFade = readPixel();
                return {beforeZoom, firstFrame, afterFade};
            } finally {
                maplibregl.restoreNow();
                maplibregl.removeProtocol('fade-test');
            }
        });

        expect(pixels.beforeZoom).toEqual([255, 0, 0, 255]);
        expect(pixels.firstFrame).toEqual([255, 0, 0, 255]);
        expect(pixels.afterFade).toEqual([0, 0, 255, 255]);
    });

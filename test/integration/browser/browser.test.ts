...
describe('Browser tests', () => {
    ...
    test('Map canvas is not blank after context lost and restored', {retry: 3, timeout: 20000}, async () => {
        const evalResult = await page.evaluate(async () => {
            ...
            return {ok: true, pixel: Array.from(rgba), x, readY, width, height, dpr};
        });

        if (!evalResult.ok) {
            console.warn('restore eval failed:', evalResult);
        }
        if (evalResult.ok && evalResult.pixel) {
            const px = evalResult.pixel as number[];

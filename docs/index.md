...
MapLibre GL JS ships an ES module build (`maplibre-gl.mjs`) alongside the classic UMD bundle. The `"module"` field in `package.json` points at the ESM bundle, so bundlers pick it up automatically. The ESM build requires a one-time [`setWorkerUrl()`](./API/functions/setWorkerUrl.md) call:
...
For setup with Vite, webpack, esbuild, or Rollup, see the [ESM guide](./guides/esm.md).

...
Micro benchmarks live next to the code they measure (`src/**/*.bench.ts`) and run with `npm run bench`; the browser harness lives in `test/bench/`. If your PR claims a performance effect, include a before/after table from `npm run bench -- --compare` in the description.

...
- Fix `getCameraAltitude()` returning `NaN` and marker terrain occlusion never applying under `globe` and `vertical-perspective` projections; `getCameraAltitude()` and `getCameraLngLat()` now follow the globe geometry while the globe is rendered ([#6584](https://github.com/maplibre/maplibre-gl-js/issues/6584)) (by [@bigmistqke](https://github.com/bigmistqke) and [@patte](https://github.com/patte))

...
- Account for `pitch` in `Map#cameraForBounds` and `Map#fitBounds` so the bounds stay in view on a pitched map, and return `pitch` in the result. Like `bearing`, `pitch` defaults to 0, so `fitBounds` now flattens a pitched map unless `pitch` is passed ([#8479](https://github.com/maplibre/maplibre-gl-js/issues/8479)) (by [@sargunv](https://github.com/sargunv))

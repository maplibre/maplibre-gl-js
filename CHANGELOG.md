...
- Fix tile selection on the globe projection measuring per-tile distances from a point that is not the camera. `coveringTiles` recovered the camera position by intersecting the camera's nadir ray with the ground, which is exact against mercator's ground plane but lands progressively farther out against the globe's sphere, so at a narrow field of view and high pitch some views were refined several zoom levels past what was requested and others left coarser than it. Mercator is unaffected ([#8187](https://github.com/maplibre/maplibre-gl-js/pull/8187)) (by [@Alchez](https://github.com/Alchez))
- _...Add new stuff here..._

...
- Add `StyleImageInterface.renderToTexture`, an escape hatch for plugin developers and advanced users that draws a style image straight onto the GPU instead of moving its pixels through the CPU. Nothing new is possible that `render` could not already do, but an image that changes every frame, such as an animated icon, gets much cheaper ([#7954](https://github.com/maplibre/maplibre-gl-js/pull/7954)) (by [@lucaswoj](https://github.com/lucaswoj))

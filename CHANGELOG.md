...
- Add the `fill-layer-blend` and `line-layer-blend` paint properties, which control how the layer is composited with the content already rendered beneath it. Supports `normal` (the default), `multiply`, `hsv-value`, `screen`, `overlay`, `plus` and `erase`. Like `{fill,line}-layer-opacity`, the blend applies to the finished layer, so overlapping features blend as a single surface instead of once per feature ([#48](https://github.com/maplibre/maplibre-gl-js/issues/48)) (by [@CommanderStorm](https://github.com/CommanderStorm))

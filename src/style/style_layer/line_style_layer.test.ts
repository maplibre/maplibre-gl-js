...
describe('LineStyleLayer', () => {
    ...
    describe('hasOffscreenPass', () => {
        ...
        test('returns false by default (line-layer-opacity defaults to 1)', () => {
            const lineLayer = createEvaluatedLineLayer({'line-color': 'red', 'line-width': 14});
            expect(lineLayer.hasOffscreenPass()).toBe(false);
        });

        test('returns true when line-layer-opacity is between 0 and 1', () => {
            const lineLayer = createEvaluatedLineLayer({'line-color': 'red', 'line-width': 14, 'line-layer-opacity': 0.5});
            expect(lineLayer.hasOffscreenPass()).toBe(true);
        });

        test('returns false when line-layer-opacity is 0', () => {
            const lineLayer = createEvaluatedLineLayer({'line-color': 'red', 'line-width': 14, 'line-layer-opacity': 0});
            expect(lineLayer.hasOffscreenPass()).toBe(false);
        });

        test('returns false when line-layer-opacity is 1', () => {
            const lineLayer = createEvaluatedLineLayer({'line-color': 'red', 'line-width': 14, 'line-layer-opacity': 1});
            expect(lineLayer.hasOffscreenPass()).toBe(false);
        });
    });
});

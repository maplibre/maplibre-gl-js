...
describe('getCameraTargetElevation', () => {
    test('Elevation is zero without terrain, and matches any given terrain', () => {
        ...
        map.transform = transform as unknown as ITransform;
...
describe('Keep camera outside terrain', () => {
    test('Try to move camera into terrain', () => {
        ...
        const initialCameraAltitude = map.transform.getCameraAltitude();
        const initialCameraLngLat = map.transform.getCameraLngLat();
        expect(initialCameraAltitude).toBeCloseTo(506, 0);
        ...
        expect(map.transform.getCameraLngLat().lng).toBeCloseTo(initialCameraLngLat.lng);
        expect(map.transform.getCameraLngLat().lat).toBeCloseTo(initialCameraLngLat.lat);
        expect(map.transform.pitch).toBeLessThan(45);
        expect(map.transform.getCameraAltitude()).toBeGreaterThan(initialCameraAltitude);

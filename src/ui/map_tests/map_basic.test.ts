...
describe('Map', () => {
    ...
    describe('is_Loaded', () => {
        ...
        test('Map.areTilesLoaded', async () => {
            ...
            map.addSource('geojson', createStyleSource());
            map.style.tileManagers.geojson.getState().addTile(new Tile(fakeTileId, undefined));
            expect(map.areTilesLoaded()).toBe(false);
            map.style.tileManagers.geojson.getState().getTileByID(fakeTileId.key).state = 'loaded';
            expect(map.areTilesLoaded()).toBe(true);
        });
    });

...
export class GeoJSONSource extends Evented implements Source {
    ...
    async getClusterExpansionZoom(clusterId: number): Promise<number> {
        return (await this.actor).sendAsync({type: MessageType.getClusterExpansionZoom, data: {type: this.type, clusterId, source: this.id}});
    }
    ...
    async getClusterChildren(clusterId: number): Promise<GeoJSON.Feature[]> {
        return (await this.actor).sendAsync({type: MessageType.getClusterChildren, data: {type: this.type, clusterId, source: this.id}});
    }
    ...
    async getClusterLeaves(clusterId: number, limit: number, offset: number): Promise<GeoJSON.Feature[]> {
        return (await this.actor).sendAsync({type: MessageType.getClusterLeaves, data: {
            type: this.type,
            source: this.id,
            clusterId,
            limit,
    ...
    private async _dispatchWorkerUpdate(optionsPromise: Promise<LoadGeoJSONParameters>) {
        ...
        try {
            ...
            const result = await (await this.actor).sendAsync({type: MessageType.loadData, data: options});
            this._isUpdatingWorker = false;
    ...
    async loadTile(tile: Tile): Promise<void> {
        ...
        const data = await (await this.actor).sendAsync({type: message, data: params}, tile.abortController);
    ...
    async unloadTile(tile: Tile): Promise<void> {
        tile.unloadVectorData();
        await (await this.actor).sendAsync({type: MessageType.removeTile, data: {uid: tile.uid, type: this.type, source: this.id}});
    }

    onRemove(): void {
        this._removed = true;
        this.actor.then(actor => actor.sendAsync({type: MessageType.removeSource, data: {type: this.type, source: this.id}}));
    }

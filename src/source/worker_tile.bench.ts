...
function parseTile(tile: {tileID: OverscaledTileID; buffer: Buffer}) {
    ...
    return workerTile.parse(vectorTile, layerIndex, [], actor, SubdivisionGranularitySetting.noSubdivision);
}

await Promise.all(tiles.map(parseTile));

bench('WorkerTile.parse', async () => {
    for (const tile of tiles) {
        await parseTile(tile);
    }
});

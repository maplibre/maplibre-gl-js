...
describe('worker tile', () => {
    ...
    test('WorkerTile.parse would request all types of dependencies', async () => {
        ...
        const sendAsync = vi.fn().mockImplementation((message: {type: string; data: any}) => {
            if (message.type === MessageType.getImages) {
                ...
                return Promise.resolve({'StandardFont-Bold': {normal: {'e': {id: 101, bitmap: {width: 1, height: 1, data: new Uint8Array([0])}, metrics: {width: 1, height: 1, left: 0, top: 0, advance: 1}}}, vertical: {}}});
        ...
        expect(sendAsync).toHaveBeenCalledWith(expect.objectContaining({type: 'GG', data: expect.objectContaining({'source': 'source', 'type': 'glyphs', 'stacks': {'StandardFont-Bold': {normal: ['t', 'e', 's'], vertical: []}}})}), expect.any(Object));
    ...
    test('WorkerTile.parse would cancel and only event once on repeated reparsing', async () => {
        ...
        expect(sendAsync).toHaveBeenCalledWith(expect.objectContaining({data: expect.objectContaining({'source': 'source', 'type': 'glyphs', 'stacks': {'StandardFont-Bold': {normal: ['t', 'e', 's'], vertical: []}}})}), expect.any(Object));
    });

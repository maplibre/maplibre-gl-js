...
describe('Validate Sprite', () => {
    ...
    test('Should return error if array contains non-object values', () => {
        ...
        errors = validateSprite({key: 'sprite', value: [3]});
        expect(errors).toHaveLength(1);
        expect(errors[0].message).toBe('sprite[0]: object expected, number found');

        errors = validateSprite({key: 'sprite', value: [{id: 'id1', url: 'url1'}, {id: 'id2', url: 'url2'}, false]});
        expect(errors).toHaveLength(1);
        expect(errors[0].message).toBe('sprite[2]: object expected, boolean found');

        errors = validateSprite({key: 'sprite', value: ['string', {id: 'id1', url: 'url1'}, {id: 'id2', url: 'url2'}, false]});
    ...
    test('Should return error if array\'s objects are not of form {id: string, url: string}', () => {
        let errors = validateSprite({key: 'sprite', value: [{id: 2, url: 2}]});
        expect(errors).toHaveLength(2);
        ...
        errors = validateSprite({key: 'sprite', value: [{test: 'string'}]});
    ...
    test('Should return error if array\'s objects contain duplicated IDs or URLs', () => {
        const errors = validateSprite({key: 'sprite', value: [{id: 'id1', url: 'url1'}, {id: 'id1', url: 'url1'}]});
        expect(errors).toHaveLength(2);
    ...
    test('Should pass if correct array of objects', () => {
        const errors = validateSprite({key: 'sprite', value: [{id: 'id1', url: 'url1'}, {id: 'id2', url: 'url2'}, {id: 'id3', url: 'url3'}]});
        expect(errors).toHaveLength(0);
    });
});

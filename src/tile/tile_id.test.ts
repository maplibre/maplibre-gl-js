...
import {CanonicalTileID, OverscaledTileID, normalizeTileCoordinates} from './tile_id';
...
describe('normalizeTileCoordinates', () => {
    test('in-bounds returns original tileID', () => {
        const tileID = new OverscaledTileID(2, 0, 2, 1, 1);
        const result = normalizeTileCoordinates(tileID, 100, 100);
    ...
    test('out-of-bounds x positive resolves right neighbor', () => {
        const tileID = new OverscaledTileID(2, 0, 2, 1, 1);
        const result = normalizeTileCoordinates(tileID, EXTENT + 100, 100);
    ...
    test('out-of-bounds y positive resolves bottom neighbor', () => {
        const tileID = new OverscaledTileID(2, 0, 2, 1, 1);
        const result = normalizeTileCoordinates(tileID, 100, EXTENT + 100);
    ...
    test('y past world bounds (poles) returns null', () => {

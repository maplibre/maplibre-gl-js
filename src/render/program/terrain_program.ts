...
const terrainUniformValues = (painter: Painter, matrix: mat4): UniformValues<TerrainUniformsType> => ({
   'u_matrix': matrix,
   'u_texture': 0,
   'u_ele_exaggeration': painter.style.terrainSourceCache.exaggeration
});

const terrainCoordsUniformValues = (painter: Painter, matrix: mat4, coordsId: number): UniformValues<TerrainCoordsUniformsType> => ({
   'u_matrix': matrix,
   'u_texture': 0,

...
function renderHeatmapFlat(painter: Painter, layer: HeatmapStyleLayer) {
    ...
    // color ramp texture.
    const fbo = layer.heatmapFbos.get('big-fb');
    if (!fbo) return;
...
function prepareHeatmapTerrain(painter: Painter, tile: Tile, layer: HeatmapStyleLayer, coord: OverscaledTileID) {
    ...
    if (!fbo) {
        fbo = createHeatmapFbo(context, tile.tileSize);
    ...
    program.draw(context, gl.TRIANGLES, DepthMode.disabled, stencilMode, colorMode, CullFaceMode.disabled,
        heatmapUniformValues(coord.posMatrix, tile, zoom, layer.paint.get('heatmap-intensity')), terrainData,
        layer.id, bucket.layoutVertexBuffer, bucket.indexBuffer,
        bucket.segments, layer.paint, painter.transform.zoom,
        programConfiguration);
}
...
function bindFramebuffer(context: Context, painter: Painter, layer: HeatmapStyleLayer) {
    ...
    let fbo = layer.heatmapFbos.get('big-fb');
    ...
    if (!fbo) {
        ...
        fbo = context.createFramebuffer(painter.width / 4, painter.height / 4, false, false);
        layer.heatmapFbos.set('big-fb', fbo);

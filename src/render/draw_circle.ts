...
function drawCircles(painter: Painter, sourceCache: SourceCache, layer: CircleStyleLayer, coords: Array<OverscaledTileID>) {
    ...
    for (let i = 0; i < coords.length; i++) {
        ...
        const bucket: CircleBucket<any> | undefined | null = (tile.getBucket(layer) as any);
        if (!bucket) continue;
    ...
    gl.bindTexture(gl.TEXTURE_2D, painter.style.terrainSourceCache.getCoordsFramebuffer(painter).colorAttachment.get());
    context.activeTexture.set(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_2D, painter.style.terrainSourceCache._coordsIndexTexture.texture);

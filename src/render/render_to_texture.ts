...
export class RenderToTexture {
    ...
    constructor(painter: Painter, terrain: Terrain) {
        ...
        this.pool = new RenderPool(painter.context, 30, terrain.renderManager.tileSize * terrain.qualityFactor);
    }
    ...
    prepareForRender(style: Style, zoom: number) {
        ...
        this._prevType = null;
        this._rttTiles = [];
        this._renderableTiles = this.terrain.renderManager.getRenderableTiles();
        ...
        for (const id in style.tileManagers) {
            ...
            for (const tileID of tileIDs) {
                const keys = this.terrain.renderManager.getTerrainCoords(tileID, terrainTileRanges);

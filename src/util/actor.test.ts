...
describe('Actor', () => {
    ...
    test('forwards responses to correct handler', async () => {
        ...
        worker.worker.actor.registerMessageHandler(WorkerMessage.getClusterExpansionZoom, async (_mapId, params) => {
            await sleep(0);
        ...
        const p1 = m1.sendAsync({type: WorkerMessage.getClusterExpansionZoom, data: {type: 'geojson', source: '', clusterId: 1729}});
        const p2 = m2.sendAsync({type: WorkerMessage.getClusterExpansionZoom, data: {type: 'geojson', source: '', clusterId: 4104}});
    ...
    test('cancel a request does not reject or resolve a promise', async () => {
        ...
        const p1 = m1.sendAsync({type: WorkerMessage.getClusterExpansionZoom, data: {type: 'geojson', source: '', clusterId: 1729}}, abortController)
            .then(() => { received = true; })
            .catch(() => { received = true; });
    ...
    test('aborting a request will successfully abort it', async () => {
        ...
        worker.worker.actor.registerMessageHandler(WorkerMessage.getClusterExpansionZoom, (_mapId, _params, handlerAbortController) => {
        ...
        let received = false;
        const abortController = new AbortController();
        m1.sendAsync({type: WorkerMessage.getClusterExpansionZoom, data: {type: 'geojson', source: '', clusterId: 1729}}, abortController)
            .then(() => { received = true; })
            .catch(() => { received = true; });
    ...
    test('cancel a request that must be queued will not call the method at all', async () => {
        ...
        let received = false;
        const abortController = new AbortController();
        const p1 = actor.sendAsync({type: WorkerMessage.getClusterExpansionZoom, data: {type: 'geojson', source: '', clusterId: 1729}, mustQueue: true}, abortController)
            .then(() => { received = true; })
            .catch(() => { received = true; });
    ...
    test('send a messege that is rejected', async () => {
        ...
        worker.worker.actor.registerMessageHandler(WorkerMessage.abortTile, () => Promise.reject(createAbortError()));

        await expect(async () => actor.sendAsync({type: WorkerMessage.abortTile, data: {} as any})).rejects.toThrow(ABORT_ERROR);
    });
    ...
    test('send a messege that must be queued, it should still arrive', async () => {
        ...
        worker.worker.actor.registerMessageHandler(WorkerMessage.getClusterExpansionZoom, () => Promise.resolve(42));

        const response = await actor.sendAsync({type: WorkerMessage.getClusterExpansionZoom, data: {} as any, mustQueue: true});
    ...
    test('send a message is not registered should throw', async () => {
        ...
        await expect(async () => actor.sendAsync({type: WorkerMessage.abortTile, data: {} as any})).rejects.toThrow(/Could not find a registered handler for.*/);
    });
    ...
    test('should not process a message with the wrong map id', async () => {
        ...
        actor.sendAsync({type: WorkerMessage.getClusterExpansionZoom, data: {} as any, targetMapId: '1'});

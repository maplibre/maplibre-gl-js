...
export function makeRequest<T>(requestParameters: RequestParameters, requestDataType?: RequestDataType): NetworkRequest<T> {
    ...
    if (fetch && /:\/\//.test(requestParameters.url) && !(/^https?:|^file:/.test(requestParameters.url))) {
        ...
        if (isWorker()) {
            ...
            // if it's not a worker, then check the protocol, and if there exists a custom handler for the protocol,
            // then execute the custom handler. Otherwise, make the request using the Fetch API

...
// Sample elevation from R16F decoded DEM texture with hardware bilinear filtering.
// The texture contains pre-decoded float elevations uploaded on the CPU side,
// so no RGB unpack is needed. gl.LINEAR handles interpolation in a single fetch.
float get_elevation(vec2 pos) {
        ...
        vec2 uv = (coord + 0.5) / (u_terrain_dim + 2.0);
        return texture(u_terrain, uv).r * u_terrain_exaggeration;
    #else
        return 0.0;
    #endif
}

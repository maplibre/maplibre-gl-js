// Line gradient shader — gradient-colored lines
// Ported from line_gradient.vertex.glsl + line_gradient.fragment.glsl
// Reference: maplibre-native LineGradientShader (webgpu/line.hpp)
...
fn vertexMain(vin: VertexInput) -> VertexOutput {
    ...
    // Compute line progress: linesofar encoded in data.zw
    // data.z holds direction in low 2 bits, linesofar high bits in upper portion
    // data.w holds additional linesofar bits
    // See maplibre-native: (data.z / 4 + data.w * 64) * 2 / MAX_LINE_DISTANCE
    let v_linesofar = (floor(f32(vin.data.z) * 0.25) + f32(vin.data.w) * 64.0) * LINE_DISTANCE_SCALE;
    vout.v_lineprogress = v_linesofar / MAX_LINE_DISTANCE;

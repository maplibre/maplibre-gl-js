...
describe('GlobeProjection', () => {
    ...
    describe('clipping plane', () => {
        ...
        describe('exact plane test', () => {
            test('looking at null island', () => {
                const mat = mat4.create();
                const transform = createMockTransform({
                    pitchDegrees: 0,
                });
                ...
                expectToBeCloseToArray(projectionData.u_projection_clipping_plane, [0, 0, 0.25, -0.13247986795120223]);
            });
            test('looking at null island, rotated', () => {
                const mat = mat4.create();
                const transform = createMockTransform({
                    pitchDegrees: 0,
                    angleDegrees: 135
            ...
            test('looking at null island, pitched', () => {
                const mat = mat4.create();
                const transform = createMockTransform({
                    pitchDegrees: 30,
                    angleDegrees: 135
                ...
                expectToBeCloseToArray(projectionData.u_projection_clipping_plane, [0.04300982560161376, 0.04300982560161375, 0.24248775186272306, -0.1371356252126768]);
            });
            test('looking at null island, pitched, rotated', () => {
                const mat = mat4.create();
                const transform = createMockTransform({
                    pitchDegrees: 30,
                    angleDegrees: 135
            ...
            test('looking at 40E15N', () => {
                ...
                console.log(projectionData.u_projection_clipping_plane);
                expectToBeCloseToArray(projectionData.u_projection_clipping_plane, [0.0030460670240555886, 0.001142311349299421, 0.2499788323047898, -0.13248051805921224]);
            });
        });
    });
});

...
const StatisticsPlot = props => {
    ...
    const t = d3.scaleLinear()
        .domain([
            Number(d3.min(summaries.map(s => s.min))),
            Number(d3.max(summaries.map(s => Math.min(s.max, s.q2 + 3 * s.iqr))))
        ])
        .range([height, 0])
        .clamp(true)
        .nice();
    ...
    const p = d3.scaleLinear()
        .domain([0, Number(d3.max(versions.map(v => d3.max(v.density, d => d[1]))))])
        .range([0, kdeWidth]);
...
const RegressionPlot = props => {
    ...
    const x = d3.scaleLinear()
        .domain([0, Number(d3.max(versions.map(version => d3.max(version.regression.data, d => d[0]))))])
        .range([0, width])
        .nice();

    const y = d3.scaleLinear()
        .domain([0, Number(d3.max(versions.map(version => d3.max(version.regression.data, d => d[1]))))])
        .range([height, 0])
        .nice();
    ...
    useEffect(() => {
        setClientWidth(ref.current.ref)
    }, [])
...
const BenchmarkStatistic = props => {
    switch (props.status) {
    case 'waiting':
...
const renderStatistic = (title, statistic, props) => {
    return (
        <tr>
            <th>{title}</th>
    ...
    return (
        <div className="col12 clearfix space-bottom">
            <table className="fixed space-bottom">
                <tbody>
                    <tr><th><h2 className="col4"><a href={`#${props.name}`} onClick={location.reload}>{props.name}</a></h2></th>
                    ...
                    {renderStatistic('(Windsorized) Deviation',
                        (version) => <p>{formatSample(version.summary.windsorizedDeviation)} ms</p>, props)}
                    {renderStatistic('R² Slope / Correlation',
                        (version) => <p>{formatSample(version.regression.slope)} ms / {version.regression.correlation.toFixed(3)} {
                            ...
                            version.regression.correlation < 0.99 ? '\u26A0\uFE0F' : ''}</p>, props)}
                    {renderStatistic('Minimum',
                        (version) => <p>{formatSample(version.summary.min)} ms</p>, props)}

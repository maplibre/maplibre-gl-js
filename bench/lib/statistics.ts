...
export function probabilitiesOfSuperiority(before, after) {
    ...
    for (const b of before) {
        for (const a of after) {
            N++;
            if (b - a > timerPrecision) superiorCount++; else if (a - b > timerPrecision) inferiorCount++; else equalCount++;
        }
    }
...
export function summaryStatistics(data) {
    ...
    const windsorizedDeviation = d3.deviation(data.map(d => d < lowerQuintile ? lowerQuintile : d > upperQuintile ? upperQuintile : d));
    return {
        mean,
        trimmedMean,
        variance,
...
export function regression(measurements) {
    ...
    for (let i = 0, n = 1; i + n < measurements.length; i += n, n++) {
        ...
        result.push([subset.reduce((sum, measurement) => sum + measurement.iterations, 0), subset.reduce((sum, measurement) => sum + measurement.time, 0)]);
    }

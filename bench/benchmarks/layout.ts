...
export default class Layout extends Benchmark {
    ...
    constructor(style: string | StyleSpecification, tileIDs?: Array<OverscaledTileID> | null) {
        super();
        this.style = style;
        this.tileIDs = tileIDs || [new OverscaledTileID(12, 0, 12, 655, 1583), new OverscaledTileID(8, 0, 8, 40, 98), new OverscaledTileID(4, 0, 4, 3, 6), new OverscaledTileID(0, 0, 0, 0, 0)];
    }
    ...
    setup(): Promise<void> {
        return fetchStyle(this.style).then(styleJSON => {
            ...
            this.tiles = tiles; // parse tiles once to populate glyph/icon cache
